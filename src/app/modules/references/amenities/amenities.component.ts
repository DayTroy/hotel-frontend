...
export class AmenitiesComponent implements OnInit {
      ...
      .pipe(
        ...
        switchMap((confirmed) => {
          ...
          return this._amenitiesApi.delete(categoryId).pipe(
            tap(() => {
              this.alerts
                .open('Дополнительная услуга успешно удалена', {
                  appearance: 'positive',
                })
                .subscribe();
            }),
  ...
  public addAmenity(content: PolymorpheusContent<TuiDialogContext>): void {
  ...
  protected submit(observer: any): void {
    if (this.amenityForm.valid) {
      ...
      const request$ = this.isEditMode
        ? this._amenitiesApi.update(formValue.amenityId, formValue)
      ...
      request$
        ...
        .subscribe(() => {
          this.alerts
            .open(
              `Дополнительная услуга успешно ${
                this.isEditMode ? 'обновлена' : 'создана'
              }`,
              { appearance: 'positive' }
            )
            .subscribe();
