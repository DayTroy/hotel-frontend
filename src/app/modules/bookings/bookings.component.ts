...
export class BookingsComponent implements OnInit {
  ...
  protected readonly columns = ['bookingId', 'roomId', 'status', 'checkInDate', 'checkOutDate', 'actions'];
  ...
  protected loadBookings(): void {
    ...
    this._bookingsApiService.getAll()
      .pipe(
  ...
  confirmBookingRemove(bookingId: string): void {
    const data: TuiConfirmData = {
      content: 'Вы действительно хотете удалить бронь? После удаления выбранной брони, она станет недоступной для просмотра',
      yes: 'Подтвердить',
      no: 'Отменить',
    };
